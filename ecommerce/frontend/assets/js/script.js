...
const SITE_SETTINGS_KEY = 'ecommerce_site_settings';

(function () {
    if (window.ecommerceAuth) return;

    const USERS_KEY = 'ecommerce_users';
    const TOKEN_KEY = 'ecommerce_auth_token';
    const RESET_KEY = 'ecommerce_reset_requests';
    ...
    function generateToken(user) {
        ...
        const signature = base64Encode(`${encodedHeader}.${encodedPayload}.ecommerce_secret_key`);
    ...
    function updateUser(email, updates) {
        ...
        if (payload && payload.email === normalized) {
            const newToken = generateToken(users[index]);
...
function applyBrandSettings(brand) {
    ...
    if (logo) {
        ...
        document.querySelectorAll('.navbar-logo, .offcanvas-logo').forEach(img => {
            if (!img.alt || img.alt.toLowerCase().includes('ecommerce')) {
                img.alt = `${name} Logo`;
            }
        });
    }
...
function updateMetaForBrand(brandName) {
    if (!brandName) return;
    const replaceBrand = (value) => value.replace(/\becommerce\b/gi, brandName);
...
function replaceBrandTextNodes(root, brandName) {
    ...
    while (node) {
        ...
        if (!parent || !skipTags.has(parent.tagName)) {
            if (/\becommerce\b/i.test(node.nodeValue)) {
                node.nodeValue = node.nodeValue.replace(/\becommerce\b/gi, brandName);
            }
        }
...
function initAccountPage() {
    ...
    const user = window.ecommerceAuth?.getCurrentUser?.();
    if (!user) {
    ...
    if (profileImage.length) {
        const storedImage = localStorage.getItem(`ecommerce_profile_image_${user.email}`);
        if (storedImage) {
    ...
    profileForm.on('submit', function(e) {
        ...
        const result = window.ecommerceAuth.updateUser(user.email, updates);
        if (!result.ok) {
    ...
    passwordForm.on('submit', function(e) {
        ...
        const result = window.ecommerceAuth.updatePassword(user.email, currentPassword, newPassword);
        if (!result.ok) {
            showAccountMessage(result.error, 'warning');
            return;
        }
    ...
    profilePictureForm.on('submit', function(e) {
        ...
        reader.onload = function(evt) {
            ...
            if (result) {
                localStorage.setItem(`ecommerce_profile_image_${user.email}`, result);
    ...
    logoutBtn.on('click', function() {
        window.ecommerceAuth.logoutUser();
        window.location.href = 'login.html';
    });
...
$(document).ready(function () {
    ...
    window.addEventListener('storage', function (event) {
        if (event.key === 'ecommerce_products') {
            productsCache = null;
            productsCacheKey = null;
    ...
    function updateProductMeta(product) {
        if (!product) return;
        const title = `${product.name} | ecommerce`;
        const description = product.description || 'Discover premium ecommerce watches and accessories.';
        const canonicalUrl = `https://ecommerce.com/products.html?${product.id != null ? `id=${product.id}` : `name=${encodeURIComponent(product.name)}`}`;
                    ...
                    <div class="col-lg-7">
                        ...
                        <div class="spec-grid">
                            ...
                            <div class="spec-card">
                                ...
                                <span class="spec-value">${product.sectionName || 'ecommerce'}</span>
                            </div>
                            <div class="spec-card">
    ...
    function renderShareButtons(product) {
        ...
        const url = `https://ecommerce.com/products.html?${product.id != null ? `id=${product.id}` : `name=${encodeURIComponent(product.name)}`}`;
        const text = encodeURIComponent(`Check out ${product.name} on ecommerce.`);
        container.html(`
    ...
    $('#completeCheckoutBtn').on('click', function() {
        const currentUser = window.ecommerceAuth?.getCurrentUser?.();
        if (!currentUser) {
...
function initNewsletterModal() {
    ...
    const dismissed = localStorage.getItem('ecommerce_newsletter_dismissed');
    const dismissedAt = parseInt(localStorage.getItem('ecommerce_newsletter_dismissed_at') || '0', 10);
    const cooldownMs = 7 * 24 * 60 * 60 * 1000;
    if (dismissed && dismissedAt && (Date.now() - dismissedAt) < cooldownMs) return;
    if (dismissed && !dismissedAt) {
        localStorage.removeItem('ecommerce_newsletter_dismissed');
    }
    ...
    $('#newsletterForm').on('submit', function (e) {
        ...
        if (!email) return;
        localStorage.setItem('ecommerce_newsletter_email', email);
        localStorage.setItem('ecommerce_newsletter_dismissed', 'true');
        localStorage.setItem('ecommerce_newsletter_dismissed_at', String(Date.now()));
